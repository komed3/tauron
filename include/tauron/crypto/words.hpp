...
constexpr std::uint32_t toWord( const Bytes& bytes, std::size_t offset ) noexcept {
...
void toWords( const Bytes& bytes, Words& words ) noexcept {
  for ( std::size_t i = 0; i < words.size(); ++i )
    words[ i ] = toWord( bytes, i * 4 );
}

constexpr void fromWord( std::uint32_t word, Bytes& bytes, std::size_t offset ) noexcept {
...
void fromWords( const Words& words, Bytes& bytes ) noexcept {
  for ( std::size_t i = 0; i < words.size(); ++i )
