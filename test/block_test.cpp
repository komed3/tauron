...
int main() {
  ...
  // 1. Normal block
  {
    const auto block = Block::build( 0x00, payload );
    const auto parsed = Block::parse( block, 32 );
    const bool passed = parsed.flag == BlockFlag::PASSED && parsed.id == 0x00 && parsed.payload == payload;
  ...
  // 2. Empty payload
  {
    ...
    const auto block = Block::build( 0x01, empty );
    const auto parsed = Block::parse( block, 32 );
    const bool passed = parsed.flag == BlockFlag::PASSED && parsed.id == 0x01 && parsed.payload.empty();
  ...
  // 3. Maximum payload
  {
    const std::vector< std::uint8_t > maximum( PAYLOAD_SIZE, 0xAA );
  ...
  // 8. Sequence size below minimum
  {
    const auto block = Block::build( 0x00, payload );
    const auto parsed = Block::parse( block, 31 );
    const bool passed = parsed.flag == BlockFlag::INVALID_ID;

    printResult( "Sequence below minimum", passed );
    allPassed &= passed;
  }
  ...
  // 11. Invalid checksum in random padding
  {
    auto block = Block::build( 0x00, payload );
  ...
  {
    bool threw = false;
    const std::vector< std::uint8_t > oversized( PAYLOAD_SIZE + 1, 0xAA );
